...
{
    public partial class AddBorrowedForm: Form
    {
        public AddBorrowedForm()
        {
            InitializeComponent();
        }
        ...
        private void CheckCode()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    var v = CheckUserCode(row.Cells["code"].Value.ToString());
        ...
        private void InsertData()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    var insert = MySQL.insert("INSERT INTO borrowed_load (user_id, summa, date, comment) VALUES ((SELECT id FROM users WHERE code = @code), @summa, @date, @comment)", parameters);
